...
def train_model(model_net_instance, epochs, lr):
    ...
    with torch.no_grad():
        ...
        mse = torch.nn.functional.mse_loss(test_predictions, y_test_t.unsqueeze(1)).item()

    return test_predictions, mse, loss_curve_values
...
for choice, config in neural_network_config.items():
    ...
    test_predictions_iter, mse_iter, loss_curve = train_model(net_instance, epochs=6000, lr=0.01)

    print(f"MSE: {mse_iter:.4f}")
